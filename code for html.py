...
##################################################################################
#NHL Schedule
nhl_schedule = pd.read_csv('2022_2023_NHL_Schedule.csv',header=None)
nhl_schedule.columns = ["Date", "Time", "Away", "Home"]
nhl_schedule.drop('Time', axis = 1)

nhl_teams = pd.read_csv('Team keys.csv',header=None)
nhl_teams.columns = ["LongName", "Ticker"]
nhl_teams.head()

nhl_schedule = nhl_schedule.merge(nhl_teams.rename({'Ticker': 'AwayTicker'}, axis=1),
               left_on='Away', right_on='LongName', how='left')
nhl_schedule = nhl_schedule.merge(nhl_teams.rename({'Ticker': 'HomeTicker'}, axis=1),
               left_on='Home', right_on='LongName', how='left')
nhl_schedule = nhl_schedule.drop(['Time','LongName_x','LongName_y'], axis = 1)
...
def run_todays_games():
    ...
    todays_games = nhl_schedule[nhl_schedule.Date == todays_date]
...
def send_emails(htmlcode):
    ...
    contacts = ['parknathan12@yahoo.com','nathan.park@drake.edu','parknick98@gmail.com']
